...
/**
 * Server-side login gate: presence check on the auth cookies set by the
 * Next API routes. Without this, the client-side RequireAuth gate still lets
 * anonymous requests fetch page HTML/RSC payloads directly. This is a
 * presence check (token validity is enforced by the backend on data fetches);
 * it exists to make every page redirect to /login before rendering anything.
 */
function hasAuthCookies(req: NextRequest): boolean {
  return Boolean(
    req.cookies.get('LH_access')?.value || req.cookies.get('LH_refresh')?.value
  )
}
...
export const config = {
  matcher: [
     ...
     * 9. /content (backend media reverse proxy — same reason as /ingest:
     *    the middleware would rewrite it into an org route and 404)
     * 10. /welcome-art (static quiz illustrations in /public — subdirectory
     *    assets are not covered by the root-file exclusion)
     */
    '/((?!api|_next|fonts|umami|ingest|content|welcome-art|examples|embed|monitoring|[\\w-]+\\.\\w+).*)',
    '/sitemap.xml',
    '/robots.txt',
...
export default async function proxy(req: NextRequest) {
  ...
  if (await isAdminSubdomain(fullhost, instance)) {
    if (!hasAuthCookies(req)) return redirectToLogin(req)

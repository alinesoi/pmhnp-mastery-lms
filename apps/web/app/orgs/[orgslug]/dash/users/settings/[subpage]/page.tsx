...
import { ClipboardList, ScanEye, SquareUserRound, UserPlus, Users, Shield } from 'lucide-react'
...
import OrgOnboarding from '@components/Dashboard/Pages/Users/OrgOnboarding/OrgOnboarding'
...
function UsersSettingsPage(props: { params: Promise<SettingsParams> }) {
  ...
  function handleLabels() {
    ...
    if (params.subpage == 'onboarding') {
      setH1Label('Onboarding')
      setH2Label('See what each member shared in the welcome quiz')
    }
  }
  ...
  const tabs: DashTabItem[] = [
    ...
    },
    {
      ...
      href: getUriWithOrg(params.orgslug, '') + `/dash/users/settings/onboarding`,
      active: params.subpage === 'onboarding',
    },
    {
      key: 'audit-logs',
  ...
  return (
    <div className="h-screen w-full bg-[#f8f8f8] grid grid-rows-[auto_1fr] grid-cols-1 overflow-hidden">
      ...
      >
        ...
        {params.subpage == 'onboarding' ? <><div className="h-6"></div><OrgOnboarding /></> : ''}
      </motion.div>
    </div>
  )
}

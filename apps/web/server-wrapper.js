...
try {
  ...
  // Browser must call the same-origin proxy paths, never the absolute
  // backend URL (breaks HTTPS/media on the public hostnames).
  const clientConfig = { ...runtimeConfig };
  clientConfig.NEXT_PUBLIC_LEARNHOUSE_API_URL = '/api/v1/';
  clientConfig.NEXT_PUBLIC_LEARNHOUSE_MEDIA_URL = '/';
  fs.writeFileSync(
    scriptPath,
    `window.__RUNTIME_CONFIG__ = ${JSON.stringify(clientConfig)};`,
    'utf8'
  );
} catch {
